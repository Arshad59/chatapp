...
def direct_chat_room(request, pk):
    ...
    if (
        not PrivateChatRoom.objects.filter(
            user1=request.user, user2=other_user
        ).exists()
        and not PrivateChatRoom.objects.filter(
            user2=request.user, user1=other_user
        ).exists()
    ):
    ...
    if PrivateChatRoom.objects.filter(user1=request.user, user2=other_user).exists():
        ...
        if request.user == direct_chat.user1:
            user1chat = PrivateChatRoom.objects.get(
                user1=request.user, user2=other_user
            )
    ...
    if PrivateChatRoom.objects.filter(user2=request.user, user1=other_user).exists():
        ...
        if request.user == direct_chat.user2:
            user2chat = PrivateChatRoom.objects.get(
                user1=other_user, user2=request.user
            )
            chat_messages = PrivateMessage.objects.filter(privateroom=direct_chat)
            return render(
                request,
                "user2directchatroom.html",
                {
                    "user2chat": user2chat,
                    "chat_messages": chat_messages,
                },
