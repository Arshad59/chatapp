...
class PrivateChatRoom(models.Model):
    ...
    def __str__(self):
        return "DirectChat between: {} and {}".format(
...
class Message(models.Model):
    ...
    def __str__(self):
        return (
            f"Public Messages: {self.user.username}: {self.content} [{self.timestamp}]"
        )
...
class PrivateMessage(models.Model):
    ...
    private_sender = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="private_sender",
    )
    private_receiver = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="private_receiver",
    )
    ...
    timestamp = models.DateTimeField(auto_now_add=True)
    is_media = models.BooleanField(default=False)
    image = models.ImageField(upload_to="private_chat_images/", null=True, blank=True)
